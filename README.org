...
- Comprehensive (e.g. tries to cover new versions from the [[https://www.ietf.org/archive/id/draft-peabody-dispatch-new-uuid-format-04.txt][IETF draft]])
...
A 48 bit unix timestamp (milliseconds), a 15 bit counter (used as a
clock sequence similarly to version 1) and 59 bits of random data are
used for generating version 7 UUID values.

The counter is reinitialized on every clock tick with the highest bit
set to zero and a random value for the remaining bits.

#+begin_src lisp
  (fuuid:make-v7)
  ;; => #<FRUGAL-UUID:UUID 01899cec-4c97-77a1-ab69-18d4cc5aa465>
#+end_src
