...
- Define the `MAKE-V3` and `MAKE-V5` functions relying on the babel
  and Ironclad libraries.
...
If you're using the `:frugal-uuid/non-frugal` system, then you can
also generate name-based (version 3 or version 5) UUIDs:
